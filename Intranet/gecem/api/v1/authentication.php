...
$app->get('/utilisateur/:id', function ($id) {
 ...
 $db = new DbHandler();
	$query="SELECT distinct id, nom, prenom, identifiant, mdp, type, id_laboratoire, id_etude FROM utilisateur where id=$id";
...
$query="select * from utilisateur where identifiant='$identifiant' and  id <>$id";
 $response =$db->execute($query);
 ...
 $query="UPDATE utilisateur SET nom='$nom', prenom='$prenom', identifiant='$identifiant', mdp='$mdp', type='$type' where id =$id;";
...
$app->get('/utilisateurs/type/:type', function ($type) {
    $db = new DbHandler();
	 ...
	 $query="SELECT distinct id, nom, prenom, identifiant, mdp, type, id_laboratoire, id_etude FROM utilisateur WHERE  type =$type order by id desc";
...
$app->get('/utilisateur/laboratoire/:id', function ($id) {
    $db = new DbHandler();
 $query="SELECT distinct id, nom, prenom, identifiant, mdp, type, id_laboratoire, id_etude FROM utilisateur where  id_laboratoire=$id order by id desc";
...
$app->get('/utilisateur/etude/:id', function ($id) {
    $db = new DbHandler();
 $query="SELECT distinct id, nom, prenom, identifiant, mdp, type, id_laboratoire, id_etude FROM utilisateur where  id_etude=$id  order by id desc";
...
$etudeId =$etudeId +"";
 ...
 foreach($fichiers as $fichier){
  $fichierLib= str_replace("$etude/","",$fichier);
...
//  Loop through each row of the worksheet in turn
    ...
    $array['cols'][] = array('type' => 'number' , 'id'=> 'Theorique','label'=> 'Theorique');
...
$array['rows'][] = 
    array('c' => array
          ( 
               array('v'=>'Date('.date('Y',PHPExcel_Shared_Date::ExcelToPHP($sheet->getCellByColumnAndRow(0, $row)->getValue())).', '.date('m',PHPExcel_Shared_Date::ExcelToPHP($sheet->getCellByColumnAndRow(0, $row)->getValue())).', '.date('d',PHPExcel_Shared_Date::ExcelToPHP($sheet->getCellByColumnAndRow(0, $row)->getValue())).')'),
